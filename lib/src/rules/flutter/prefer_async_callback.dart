...
class PreferAsyncCallbackRule extends AnalysisRule {
...
class _Visitor extends SimpleAstVisitor<void> {
  ...
  void visitGenericFunctionType(GenericFunctionType node) {
    if (!_isFutureVoidFunctionType(node)) return;

    rule.reportAtNode(node);
  }

  bool _isFutureVoidFunctionType(GenericFunctionType node) {
    if (!_isFutureVoid(node)) return false;
  ...
  bool _isFutureVoid(GenericFunctionType node) {
    final returnType = node.returnType;
    if (returnType is! NamedType) return false;
    if (returnType.type?.isDartAsyncFuture == false) return false;
    if (returnType.typeArguments?.arguments.firstOrNull?.type is! VoidType) return false;
...
class ReplaceWithAsyncCallback extends ResolvedCorrectionProducer {
  static const _fixKind = FixKind(
    'dart.fix.replaceWithAsyncCallback',
  ...
  Future<void> compute(ChangeBuilder builder) async {
    ...
    replacement = node.question == null ? 'AsyncCallback' : 'AsyncCallback?';
