...
To enable the `class_members_ordering` rule, add `class_members_ordering` under `plugins > pyramid_lint > diagnostics` in your `analysis_options.yaml` file:

```yaml
plugins:
  pyramid_lint: <version>
    diagnostics:
      class_members_ordering: true
```
