# avoid_inverted_boolean_expressions

| Severity | Quick Fix | Options |
|:--------:|:---------:|:-------:|
|   Info   |    ✅     |   ❌    |

## Details

**DO** avoid using inverted boolean expressions unnecessarily.

Unnecessary inverted boolean expression should be avoided since it decreases code readability.

```dart title="Bad"
if (!(number > 0)) {}
final text = !(number == 0) ? 'Not zero' : 'Zero';
```

```dart title="Good"
if (number <= 0) {}
final text = number != 0 ? 'Not zero' : 'Zero';
```

## Usage

To enable the `avoid_inverted_boolean_expressions` rule, add `avoid_inverted_boolean_expressions` under `plugins > pyramid_lint > diagnostics` in your `analysis_options.yaml` file:

```yaml
plugins:
  pyramid_lint: <version>
    diagnostics:
      avoid_inverted_boolean_expressions: true
```
