...
**PREFER** using dedicated MediaQuery methods instead of accessing the properties from MediaQuery.of(context) or MediaQuery.maybeOf(context).
...
To enable the `prefer_dedicated_media_query_method` rule, add `prefer_dedicated_media_query_method` under custom_lint > rules in your `analysis_options.yaml` file:
